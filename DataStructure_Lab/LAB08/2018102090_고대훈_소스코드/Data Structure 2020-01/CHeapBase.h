...
template<typename T>
inline void CHeapBase<T>::Replace(T item)
{
	Delete(item);
	Add(item);
}
...
template<typename T>
inline void CHeapBase<T>::ResetList()
{
	currentPointer = -1;
}

template<typename T>
inline int CHeapBase<T>::GetNextItem(T& item)
{
	currentPointer++;
	if (currentPointer == m_iLastNode) return 0;
	item = m_pHeap[currentPointer];
	return 1;
}
